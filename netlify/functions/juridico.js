...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (action === 'savefile') {
      const body = JSON.parse(event.body);
      const store = getReqStore();
      await store.setJSON('file-' + body.id, {
    ...
    if (action === 'getkey') {
      ...
      return { statusCode: 200, headers: CORS, body: JSON.stringify({ key: key }) };
    }

    if (action === 'getfile') {
      const fid = (event.queryStringParameters || {}).fileid;
      if (!fid) return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'fileid obrigatorio' }) };
      const file = await store.get('file-' + fid, { type: 'json' });
      if (!file) return { statusCode: 404, headers: CORS, body: JSON.stringify({ error: 'Arquivo nao encontrado' }) };
